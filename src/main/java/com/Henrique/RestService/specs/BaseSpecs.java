...
public abstract class BaseSpecs {
	...
	public static <T> Predicate toAndArray(CriteriaBuilder builder, Collection<Predicate> predicate, Collection<Predicate> predicates) {
	...
	public static <T> Predicate toOrArray(CriteriaBuilder builder, Collection<Predicate> predicate, Collection<Predicate> predicates) {
		predicates.removeIf(item -> item == null);
		if (predicates.isEmpty()) {
			return null;
		}
	...
	public static <T> Predicate equal(CriteriaBuilder buider, Expression<?> path, Optional<?> filterOpt, CriteriaBuilder builder) {
